...
impl DirPlayer {
    ...
    pub fn begin_score_sprites(
        ...
        let player = self;
        macro_rules! owner_ref { ($callback:expr) => { ($callback)(player) }; }
        macro_rules! owner_mut { ($callback:expr) => { ($callback)(player) }; }
        ...
        let dir_version = owner_ref!(|player| player.movie.dir_version);
        ...
        for (i, (span, channel_index, data)) in span_init_data.iter().enumerate() {
            ...
            let sprite: &mut Sprite = player.owned_score_mut(&score_ref).unwrap().get_sprite_mut(sprite_num);
            sprite.entered = true;
            ...
            if is_sprite {
                ...
                match &score_ref {
                    ScoreRef::Stage => {
                        let _ = owner_mut!(|player| sprite_set_prop(player, symbols, sprite_num, Symbol::builtin(BuiltInSymbol::Member), Datum::CastMember(member.clone())));
                    }
                    ScoreRef::FilmLoop(_) => {
        ...
        if dir_version >= 600 {
            ...
            for (sprite_num, member) in member_updates {
                match &score_ref {
                    ScoreRef::Stage => {
                        let _ = owner_mut!(|player| sprite_set_prop(player, symbols, sprite_num, Symbol::builtin(BuiltInSymbol::Member), Datum::CastMember(member)));
                    }
                    ScoreRef::FilmLoop(_) => {
        ...
        let sound_by_frame_channel: HashMap<(u32, u16), u8> = player.owned_score(&score_ref).unwrap().sound_channel_data.iter()
            .map(|(frame_idx, ch_idx, data)| ((*frame_idx, *ch_idx), data.cast_member))
            .collect();

        for (frame_index, channel_index, sound_data) in player.owned_score(&score_ref).unwrap().sound_channel_data.iter() {
            if *frame_index + 1 == frame_num {
...
&actual_instance_ref,
        ...
        for channel in player.owned_score(&score_ref).unwrap().channels.iter() {
        ...
        for (channel_num, data) in &sprites_to_init {
            let sprite = player.owned_score_mut(&score_ref).unwrap().get_sprite_mut(*channel_num);
            sprite.entered = true;
