...
browser_e2e_test!(
    test_browser_handle_flash_scripted_access_owner_capabilities,
    |player| async move { player.test_flash_scripted_access_owner_capabilities().await }
);
...
browser_e2e_test!(
    test_flash_initial_access_before_reservation,
    |player| async move { player.test_flash_initial_access_before_reservation().await }
);

browser_e2e_test!(
    test_flash_sprite_variable_owned_fixture,
    |player| async move { player.test_flash_sprite_variable_owned_fixture().await }
);
...
fn test_nested_flash_fixture_uses_production_director_parser() {
    for (name, bytes) in [
        ...
        .unwrap_or_else(|error| {
            panic!("{name} did not parse through read_director_file_bytes: {error}")
        });
        assert_eq!(
            parsed.base_path.as_str(),
            "https://fixture.invalid/",
            "{name} base URL"
        );
        assert_eq!(
            parsed.version, 500,
            "{name} should use the authored D5 human version"
        );
        assert_eq!(
            parsed.cast_entries.len(),
            1,
            "{name} should expose one cast entry"
        );
        assert!(
            parsed.key_table.is_some(),
            "{name} should expose its KEY* table"
        );
        let score = parsed
            .score
            .as_ref()
            .expect("{name} should expose its VWSC score");
