...
fn datum_to_mcp_value_with_options(
    ...
    let datum_id = match datum_ref {
        ...
        super::DatumRef::Ref(_) => Some(datum_ref.unwrap()),
    };

    // For compact mode, use short representations for complex types
    let value = if compact {
        format_datum_compact(datum, player, symbols, datum_id)?
    } else {
        format_concrete_datum_with_depth(datum, symbols, player, 0, 0)?
    };
...
fn format_datum_compact(
    ...
    Ok(match datum {
        ...
        Datum::Symbol(s) => format!("#{}", symbol_name(symbols, s)?),
...
fn get_script_info(script: &Script, symbols: &SymbolTable) -> Result<McpScriptInfo, ScriptError> {
    Ok(McpScriptInfo {
...
fn mcp_list_scripts_result(
    ...
    let all_scripts: Vec<McpScriptInfo> = player
        ...
        .flat_map(|cast| cast.scripts.values().map(|s| get_script_info(s, symbols)))
        .collect::<Result<_, _>>()?;
...
fn mcp_get_call_stack_result(
    ...
    let scopes: Vec<McpScopeInfo> = scopes_to_process
        .into_iter()
        .map(|index| {
            let (script_ref, handler_name_id, bytecode_index, stack_depth, locals, args) = {
                let scope = player.scopes.get_mut(index).unwrap();
                (
            ...
            let handler_name = get_handler_name(lctx, handler_name_id);
            let lctx_names = lctx.map(|context| context.names.clone());
            drop(lctx);

            Ok(McpScopeInfo {
                index,
                script_name,
                cast_lib: script_ref.cast_lib,
                cast_member: script_ref.cast_member,
                handler_name,
                bytecode_index,
                locals: if let Some(local_values) = locals {
                    Some({
...
pub fn mcp_get_globals(player: &DirPlayer, symbols: &SymbolTable) -> String {
...
fn mcp_get_locals_result(
    ...
    let arg_names: Vec<String> = player
        ...
        .cast_manager
        .get_script_by_ref(&script_ref)
        .and_then(|script| script.get_own_handler_by_local_name_id(handler_name_id))
        .map(|handler| get_argument_names(lctx, &handler.argument_name_ids))
        .unwrap_or_default();
    drop(lctx);

    Ok(to_json(&McpLocalsResult {
        scope_index: index,
        handler_name,
        locals: {
            let local_name_ids = player
...
fn mcp_inspect_datum_result(
    ...
    Ok(to_json(&McpDatumInspection {
        datum_id,
        type_name: datum.type_str().to_string(),
        value: format_concrete_datum_with_depth(datum, symbols, player, 0, 0)?,
        properties,
    }))
}
...
fn mcp_inspect_cast_member_result(
    ...
    let cast = match player.movie.cast_manager.get_cast(cast_lib as u32) {
        Ok(c) => c,
        Err(_) => return Ok(mcp_error(format!("Cast library {} not found", cast_lib))),
    };
    ...
    Ok(to_json(&McpCastMemberDetails {
        cast_lib,
        cast_member,
        name: member.name.clone(),
        member_type: member.member_type.type_string().to_string(),
...
pub fn mcp_format_eval_result(
    player: &DirPlayer,
    symbols: &SymbolTable,
    result: Result<super::DatumRef, ScriptError>,
) -> String {
    let formatted = result.and_then(|datum_ref| datum_to_mcp_value(player, symbols, &datum_ref));
    match formatted {
        Ok(value) => to_json(&McpEvalResult {
            success: true,
            result_type: Some(value.type_name),
            result_value: Some(value.value),
            datum_id: value.datum_id,
            error: None,
        }),
        Err(error) => to_json(&McpEvalResult {
            success: false,
            result_type: None,
            result_value: None,
            datum_id: None,
