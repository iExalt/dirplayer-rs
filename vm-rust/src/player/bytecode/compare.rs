...
impl CompareBytecodeHandler {
    pub fn gt(
        ...
        runtime.with_player_and_symbols(|player, symbols| {
            ...
            let is_gt = datum_greater_than(
                player.get_datum(&left),
                player.get_datum(&right),
                &player.allocator,
                symbols,
            )?;
...
mod tests {
    ...
    fn symbol_fast_paths_validate_owner_before_identity_compare() {
        ...
        player.scopes[slot]
            .stack
            .push_value(StackDatum::Symbol(local.clone()));
        player.scopes[slot]
            .stack
            .push_value(StackDatum::Symbol(local));
        ...
        assert!(matches!(
            CompareBytecodeHandler::eq(&mut runtime, &ctx),
            Ok(HandlerExecutionResult::Advance)
        ));
        assert_eq!(player.allocator.datum_count(), datum_count_before);
        assert!(matches!(
            player.scopes[slot].stack.pop_value(),
            Some(StackDatum::Int(1))
        ));
        player.pop_scope();
        ...
        player.scopes[slot]
            .stack
            .push_value(StackDatum::Symbol(left));
        player.scopes[slot]
            .stack
            .push_value(StackDatum::Symbol(right));
        let ctx = make_context(&player, slot);
        let mut runtime = ExecutionContext {
            player_id: 4,
            symbols: &mut symbols,
        ...
        assert!(matches!(
            CompareBytecodeHandler::nt_eq(&mut runtime, &ctx),
            Ok(HandlerExecutionResult::Advance)
        ));
        assert!(matches!(
            player.scopes[slot].stack.pop_value(),
        ...
        player.scopes[slot]
            .stack
            .push_value(StackDatum::Symbol(foreign.clone()));
        player.scopes[slot]
            .stack
            .push_value(StackDatum::Symbol(foreign));
        let ctx = make_context(&player, slot);
        let mut runtime = ExecutionContext {
            player_id: 4,
            symbols: &mut symbols,
