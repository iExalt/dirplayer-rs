...
use super::physx_gu_mesh::{
    box_vs_triangle, capsule_vs_triangle, sphere_vs_triangle, GuTriContact, MeshHitCallback,
};
...
impl GuHeightField {
    ...
    pub fn compute_aabb(&mut self) {
        if self.rows == 0 || self.columns == 0 {
            self.aabb_min = self.origin;
            self.aabb_max = self.origin;
            return;
        }
    ...
    pub fn intersect_aabb<C: MeshHitCallback + ?Sized>(
        &self,
        box_min: [f32; 3],
        box_max: [f32; 3],
        cb: &mut C,
        ...
        if row0_f > row1_f {
            std::mem::swap(&mut row0_f, &mut row1_f);
        }
        if col0_f > col1_f {
            std::mem::swap(&mut col0_f, &mut col1_f);
        }
        ...
        for row in row0..=row1 {
            ...
            for col in col0..=col1 {
                ...
                for sub_tri in 0..2u32 {
                    ...
                    if min_y > box_max[1] || max_y < box_min[1] {
                        continue;
                    }
                    if !cb.process(tri_idx, v0, v1, v2) {
                        return;
                    }
                }
            }
...
pub fn box_vs_heightfield(
    ...
    impl<'a> MeshHitCallback for Cb<'a> {
        fn process(&mut self, ti: u32, v0: [f32; 3], v1: [f32; 3], v2: [f32; 3]) -> bool {
            ...
            if let Some(c) = box_vs_triangle(
                self.c, self.he, self.ax, self.ay, self.az, self.cd, v0, v1, v2, ti, 0,
            ) {
                self.out.push(c);
            }
            true
