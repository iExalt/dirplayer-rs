...
pub fn ray_aabb_intersect2(
    ...
    for i in 0..3 {
        if ro[i] < minimum[i] {
            ...
            inside = false;
            if rd[i] != 0.0 {
                max_t[i] = (minimum[i] - ro[i]) / rd[i];
            }
        } else if ro[i] > maximum[i] {
            c[i] = maximum[i];
            inside = false;
            if rd[i] != 0.0 {
                max_t[i] = (maximum[i] - ro[i]) / rd[i];
            }
        }
    }
    ...
    if max_t[1] > max_t[which_plane] {
        which_plane = 1;
    }
    if max_t[2] > max_t[which_plane] {
        which_plane = 2;
    }
...
pub fn raycast_box(
    ...
    let local_origin_rel = [
        ray_origin[0] - box_pos[0],
        ray_origin[1] - box_pos[1],
        ray_origin[2] - box_pos[2],
    ];
    ...
    let (rval, t, local_impact) =
        ray_aabb_intersect2(neg, box_half_extents, local_origin, local_dir);
    if rval == 0 || t > max_dist {
        return None;
    }

    let mut hit = GuRaycastHit {
        distance: t,
        face_index: 0xffffffff,
        ..Default::default()
    };
    if t != 0.0 {
...
pub fn raycast_sphere(
    ...
    let (dist, pos) =
        intersect_ray_sphere(ray_origin, ray_dir, max_dist, sphere_center, sphere_radius)?;
    let mut hit = GuRaycastHit {
        distance: dist,
        position: pos,
        face_index: 0xffffffff,
...
pub fn raycast_capsule(
    ...
    if t == 0.0 {
        ...
        let n = [
            position[0] - p_on_seg[0],
            position[1] - p_on_seg[1],
            position[2] - p_on_seg[2],
        ];
        let nl = (n[0] * n[0] + n[1] * n[1] + n[2] * n[2]).sqrt();
        hit.normal = if nl > 1e-10 {
...
fn intersect_ray_capsule(
    ...
    if f_w_length != 0.0 {
        kw = [kw[0] / f_w_length, kw[1] / f_w_length, kw[2] / f_w_length];
    }

    // Degenerate capsule (sphere) — switch to sphere path.
    if f_w_length <= 1e-6 {
        let d0 =
            (origin[0] - p0[0]).powi(2) + (origin[1] - p0[1]).powi(2) + (origin[2] - p0[2]).powi(2);
        let d1 =
            (origin[0] - p1[0]).powi(2) + (origin[1] - p1[1]).powi(2) + (origin[2] - p1[2]).powi(2);
    ...
    let f_inv_d_length = if f_d_length != 0.0 {
        1.0 / f_d_length
    } else {
        0.0
    };
