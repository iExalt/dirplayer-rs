...
pub const BOX_EDGES: [u8; 24] = [
    0, 1, 1, 2, 2, 3, 3, 0, 7, 6, 6, 5, 5, 4, 4, 7, 1, 5, 6, 2, 3, 7, 4, 0,
];
...
fn face_region(
    ...
    if rk_dir[i0] * k_ppe[i1] >= rk_dir[i1] * rk_pme[i0] {
        if rk_dir[i0] * k_ppe[i2] >= rk_dir[i2] * rk_pme[i0] {
            ...
            f_tmp = f_lsqr * k_ppe[i1]
                - rk_dir[i1] * (rk_dir[i0] * rk_pme[i0] + rk_dir[i2] * k_ppe[i2]);
            if f_tmp <= 2.0 * f_lsqr * extents[i1] {
                ...
                *rf_sqr_distance += rk_pme[i0] * rk_pme[i0]
                    + f_tmp * f_tmp
                    + k_ppe[i2] * k_ppe[i2]
                    + f_delta * f_param;
                if want_param {
                    *pf_lparam = f_param;
                    rk_pnt[i0] = extents[i0];
                    rk_pnt[i1] = f_t - extents[i1];
                    rk_pnt[i2] = -extents[i2];
                }
            } else {
                f_lsqr += rk_dir[i1] * rk_dir[i1];
                f_delta =
                    rk_dir[i0] * rk_pme[i0] + rk_dir[i1] * rk_pme[i1] + rk_dir[i2] * k_ppe[i2];
                f_param = -f_delta / f_lsqr;
                *rf_sqr_distance += rk_pme[i0] * rk_pme[i0]
                    + rk_pme[i1] * rk_pme[i1]
                    + k_ppe[i2] * k_ppe[i2]
        ...
        f_tmp =
            f_lsqr * k_ppe[i2] - rk_dir[i2] * (rk_dir[i0] * rk_pme[i0] + rk_dir[i1] * k_ppe[i1]);
        if f_tmp <= 2.0 * f_lsqr * extents[i2] {
            ...
            f_param = -f_delta / f_lsqr;
            *rf_sqr_distance +=
                rk_pme[i0] * rk_pme[i0] + k_ppe[i1] * k_ppe[i1] + f_tmp * f_tmp + f_delta * f_param;
            if want_param {
                *pf_lparam = f_param;
                rk_pnt[i0] = extents[i0];
                rk_pnt[i1] = -extents[i1];
                rk_pnt[i2] = f_t - extents[i2];
            }
        } else {
            f_lsqr += rk_dir[i2] * rk_dir[i2];
        ...
        f_lsqr = rk_dir[i0] * rk_dir[i0] + rk_dir[i2] * rk_dir[i2];
        f_tmp =
            f_lsqr * k_ppe[i1] - rk_dir[i1] * (rk_dir[i0] * rk_pme[i0] + rk_dir[i2] * k_ppe[i2]);
        if f_tmp >= 0.0 {
            if f_tmp <= 2.0 * f_lsqr * extents[i1] {
                f_t = f_tmp / f_lsqr;
                f_lsqr += rk_dir[i1] * rk_dir[i1];
        ...
        if f_tmp >= 0.0 {
            if f_tmp <= 2.0 * f_lsqr * extents[i2] {
                ...
                *rf_sqr_distance += rk_pme[i0] * rk_pme[i0]
                ...
                f_lsqr += rk_dir[i2] * rk_dir[i2];
                f_delta =
                    rk_dir[i0] * rk_pme[i0] + rk_dir[i1] * k_ppe[i1] + rk_dir[i2] * rk_pme[i2];
                f_param = -f_delta / f_lsqr;
                *rf_sqr_distance += rk_pme[i0] * rk_pme[i0]
                    + k_ppe[i1] * k_ppe[i1]
                    + rk_pme[i2] * rk_pme[i2]
    ...
    if f_prod0 >= f_prod1 {
        ...
        if f_delta >= 0.0 {
            ...
            *rf_sqr_distance += f_delta * f_delta * f_inv_lsqr;
            if want_param {
                rk_pnt[i1] = -extents[i1];
                *pf_lparam = -(rk_dir[i0] * f_pme0 + rk_dir[i1] * f_ppe1) * f_inv_lsqr;
            }
        } else if want_param {
            let f_inv = 1.0 / rk_dir[i0];
            rk_pnt[i1] -= f_prod0 * f_inv;
            *pf_lparam = -f_pme0 * f_inv;
        }
    } else {
        rk_pnt[i1] = extents[i1];
        ...
        if f_delta >= 0.0 {
            ...
            *rf_sqr_distance += f_delta * f_delta * f_inv_lsqr;
            if want_param {
                rk_pnt[i0] = -extents[i0];
                *pf_lparam = -(rk_dir[i0] * f_ppe0 + rk_dir[i1] * f_pme1) * f_inv_lsqr;
            }
        } else if want_param {
            let f_inv = 1.0 / rk_dir[i1];
            rk_pnt[i0] -= f_prod1 * f_inv;
            *pf_lparam = -f_pme1 * f_inv;
        }
    }
    if rk_pnt[i2] < -extents[i2] {
        f_delta = rk_pnt[i2] + extents[i2];
        *rf_sqr_distance += f_delta * f_delta;
        rk_pnt[i2] = -extents[i2];
    } else if rk_pnt[i2] > extents[i2] {
        f_delta = rk_pnt[i2] - extents[i2];
        *rf_sqr_distance += f_delta * f_delta;
        rk_pnt[i2] = extents[i2];
    }
...
fn case00(
    ...
    if want_param {
        *pf_lparam = (extents[i0] - rk_pnt[i0]) / rk_dir[i0];
    }
    rk_pnt[i0] = extents[i0];
    let mut f_delta;
    if rk_pnt[i1] < -extents[i1] {
        f_delta = rk_pnt[i1] + extents[i1];
        *rf_sqr_distance += f_delta * f_delta;
        rk_pnt[i1] = -extents[i1];
    } else if rk_pnt[i1] > extents[i1] {
        f_delta = rk_pnt[i1] - extents[i1];
        *rf_sqr_distance += f_delta * f_delta;
        rk_pnt[i1] = extents[i1];
    }
...
fn case000(rk_pnt: &mut [f64; 3], extents: &[f64; 3], rf_sqr_distance: &mut f64) {
    let mut f_delta;
    if rk_pnt[0] < -extents[0] {
        f_delta = rk_pnt[0] + extents[0];
        *rf_sqr_distance += f_delta * f_delta;
        rk_pnt[0] = -extents[0];
    } else if rk_pnt[0] > extents[0] {
        f_delta = rk_pnt[0] - extents[0];
        *rf_sqr_distance += f_delta * f_delta;
        rk_pnt[0] = extents[0];
    }
    if rk_pnt[1] < -extents[1] {
        f_delta = rk_pnt[1] + extents[1];
        *rf_sqr_distance += f_delta * f_delta;
        rk_pnt[1] = -extents[1];
    } else if rk_pnt[1] > extents[1] {
        f_delta = rk_pnt[1] - extents[1];
        *rf_sqr_distance += f_delta * f_delta;
        rk_pnt[1] = extents[1];
    }
    if rk_pnt[2] < -extents[2] {
        f_delta = rk_pnt[2] + extents[2];
        *rf_sqr_distance += f_delta * f_delta;
        rk_pnt[2] = -extents[2];
    } else if rk_pnt[2] > extents[2] {
        f_delta = rk_pnt[2] - extents[2];
        *rf_sqr_distance += f_delta * f_delta;
        rk_pnt[2] = extents[2];
    }
...
fn distance_line_box_squared(
    ...
    let mut dir = [
        line_direction[0] * box_rot.col0[0]
            + line_direction[1] * box_rot.col0[1]
            + line_direction[2] * box_rot.col0[2],
        line_direction[0] * box_rot.col1[0]
            + line_direction[1] * box_rot.col1[1]
            + line_direction[2] * box_rot.col1[2],
        line_direction[0] * box_rot.col2[0]
            + line_direction[1] * box_rot.col2[1]
            + line_direction[2] * box_rot.col2[2],
    ];
    ...
    if dir[0] > 0.0 {
        if dir[1] > 0.0 {
            if dir[2] > 0.0 {
                case_no_zeros(&mut pnt, &dir, &ext, &mut lp, want_param, &mut sqr);
            } else {
                case0(0, 1, 2, &mut pnt, &dir, &ext, &mut lp, want_param, &mut sqr);
            }
        } else if dir[2] > 0.0 {
            case0(0, 2, 1, &mut pnt, &dir, &ext, &mut lp, want_param, &mut sqr);
        } else {
            case00(0, 1, 2, &mut pnt, &dir, &ext, &mut lp, want_param, &mut sqr);
        }
    } else if dir[1] > 0.0 {
        if dir[2] > 0.0 {
            case0(1, 2, 0, &mut pnt, &dir, &ext, &mut lp, want_param, &mut sqr);
        } else {
            case00(1, 0, 2, &mut pnt, &dir, &ext, &mut lp, want_param, &mut sqr);
        }
    } else if dir[2] > 0.0 {
        case00(2, 0, 1, &mut pnt, &dir, &ext, &mut lp, want_param, &mut sqr);
    } else {
        case000(&mut pnt, &ext, &mut sqr);
        lp = 0.0;
    }
...
pub fn distance_segment_box_squared(
    ...
    let (sqr, lp, bp) = distance_line_box_squared(
        seg_p0,
        v_sub(seg_p1, seg_p0),
        box_origin,
        box_extent,
...
fn gu_test_axis(
    ...
    if min0 > max0 {
        std::mem::swap(&mut min0, &mut max0);
    }
    min0 -= radius;
    max0 += radius;

    let box_cen = v_dot(b.center, axis);
    let box_ext = v_dot(b.col0, axis).abs() * b.extents[0]
...
fn gu_generate_vf_contacts(
    ...
    for i in 0..2 {
        ...
        if res != -1 && tnear < radius + contact_distance {
            contact_buffer.add(v_sub(pos, v_mul(normal, tnear)), normal, tnear - radius);
        }
    }
}
...
fn gu_generate_ee_contacts_impl(
    ...
    for i in 0..12 {
        ...
        if let Some((dist, ip)) =
            intersect_edge_edge_preca(s0, s1, v1, plane_n, plane_d, ii, jj, coeff, dir_arg, p1, p2)
        {
            if !use_v2_negated {
                contact_buffer.add(v_sub(ip, v_mul(normal, dist)), normal, -(radius + dist));
            } else {
                if check_contact_distance && dist >= radius + contact_distance {
                    continue;
                }
                contact_buffer.add(v_sub(ip, v_mul(normal, dist)), normal, dist - radius);
            }
        }
    }
}
...
pub fn contact_capsule_box(
    ...
    if square_dist >= inflated_radius * inflated_radius {
        return 0;
    }
    ...
    if !penetration {
        ...
        if normal_len > 0.0 {
            ...
            if contact_buffer.count() - count_before == 2 {
                return 2;
            }
            gu_generate_ee_contacts2(
                contact_buffer,
                seg_p0,
                seg_p1,
                capsule_radius,
    ...
    if penetration {
        let Some((depth, sep_axis)) =
            gu_capsule_obb_overlap3(seg_p0, seg_p1, capsule_radius, &world_box)
        else {
            return 0;
        };
        gu_generate_vf_contacts(
