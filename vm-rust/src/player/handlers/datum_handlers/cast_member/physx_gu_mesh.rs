...
use super::physx_gu_rtree::{
    LeafTriangles, RTree, RTreeAabbCallback, RTreeBuilder, RTreeRaycastCallback,
};
        ...
        for users in edges.values() {
            ...
            for i in 0..users.len() {
                for j in (i + 1)..users.len() {
                    let (ti, ei) = users[i];
                    let (tj, ej) = users[j];
                    let (Some(ni), Some(nj)) = (normals[ti], normals[tj]) else {
                        continue;
                    };
...
pub fn midphase_intersect_aabb<C: MeshHitCallback + ?Sized>(
    ...
    let mut adapter = MidphaseAabbAdapter {
        mesh,
        cb,
        aborted: false,
    };
...
pub fn sphere_vs_triangle(
    ...
    if dot(plane_normal, sphere_center) < plane_d {
        return None;
    }
...
fn closest_pt_segment_segment(
    ...
    if a <= K_EPS {
        ...
        if e <= K_EPS {
            ...
            let mut s_calc = if denom != 0.0 {
                ((bdot * f - cdot * e) / denom).clamp(0.0, 1.0)
            } else {
                0.0
            };
...
pub fn capsule_vs_triangle(
    ...
    let plane_normal = cross(e0, e1);
    let plane_d = dot(plane_normal, v0);
    if dot(plane_normal, seg_pt) < plane_d {
        return None;
    }
...
pub fn box_vs_triangle(
    ...
    if dot(tri_normal, box_center) < dot(tri_normal, v0) - box_reach_along_n - contact_dist {
        return None;
    }
    ...
    let sx = if dot(box_axis_x, n) >= 0.0 {
        -box_half_extents[0]
    } else {
        box_half_extents[0]
    };
    let sy = if dot(box_axis_y, n) >= 0.0 {
        -box_half_extents[1]
    } else {
        box_half_extents[1]
    };
    let sz = if dot(box_axis_z, n) >= 0.0 {
        -box_half_extents[2]
    } else {
        box_half_extents[2]
    };
...
pub fn box_vs_triangle_manifold(
    ...
    let tri_n = if dot(tri_n, single.normal) < 0.0 {
        scale(tri_n, -1.0)
    } else {
        tri_n
    };
    ...
    let step = if dot(axes[fi], tri_n) > 0.0 {
        -box_half_extents[fi]
    } else {
        box_half_extents[fi]
    };
    ...
    for c in corners.iter() {
        ...
        if !point_over_triangle(*c, v0, v1, v2, tri_n) {
            continue;
        }
...
fn test_axis(
    ...
    if box_max + contact_dist < tri_min {
        return false;
    }
    if tri_max + contact_dist < box_min {
        return false;
    }

    let depth_a = box_max - tri_min;
    let depth_b = tri_max - box_min;
    let (depth, sign) = if depth_a < depth_b {
        (depth_a, -1.0f32)
    } else {
        (depth_b, 1.0f32)
    };
...
pub fn sphere_vs_mesh(
    ...
    struct Cb<'a> {
        ...
        radius: f32,
        contact_dist: f32,
        out: &'a mut Vec<GuTriContact>,
    }
    impl<'a> MeshHitCallback for Cb<'a> {
        fn process(&mut self, ti: u32, v0: [f32; 3], v1: [f32; 3], v2: [f32; 3]) -> bool {
            if let Some(c) =
                sphere_vs_triangle(self.center, self.radius, self.contact_dist, v0, v1, v2, ti)
            {
                self.out.push(c);
            }
            true
...
pub fn capsule_vs_mesh(
    ...
    impl<'a> MeshHitCallback for Cb<'a> {
        fn process(&mut self, ti: u32, v0: [f32; 3], v1: [f32; 3], v2: [f32; 3]) -> bool {
            if let Some(c) = capsule_vs_triangle(
                self.p0,
                self.p1,
                self.radius,
                self.contact_dist,
...
pub fn box_vs_mesh(
    ...
    let mut out = Vec::new();
    let ex = box_half_extents[0] * box_axis_x[0].abs()
        + box_half_extents[1] * box_axis_y[0].abs()
        + box_half_extents[2] * box_axis_z[0].abs();
    let ey = box_half_extents[0] * box_axis_x[1].abs()
        + box_half_extents[1] * box_axis_y[1].abs()
        + box_half_extents[2] * box_axis_z[1].abs();
    let ez = box_half_extents[0] * box_axis_x[2].abs()
        + box_half_extents[1] * box_axis_y[2].abs()
        + box_half_extents[2] * box_axis_z[2].abs();
    ...
    impl<'a> MeshHitCallback for Cb<'a> {
        fn process(&mut self, ti: u32, v0: [f32; 3], v1: [f32; 3], v2: [f32; 3]) -> bool {
            ...
            box_vs_triangle_manifold(
                self.c, self.he, self.ax, self.ay, self.az, self.cd, v0, v1, v2, ti, mask, self.out,
            );
            true
        }
    }
...
pub fn raycast_mesh(
    ...
    impl<'a> RTreeRaycastCallback for Cb<'a> {
        fn process(&mut self, leaf_encoded: u32, max_t: &mut f32) -> bool {
            ...
            for k in 0..lf.nb_triangles() {
                ...
                let (v0, v1, v2) = self.mesh.get_triangle(orig_tri);
                if let Some((t, n)) =
                    ray_vs_triangle(self.ray_origin, self.ray_dir, v0, v1, v2, *max_t)
                {
                    if t < self.best_t {
                        self.best_t = t;
                        let p = [
    ...
    mesh.tree
        .traverse_ray(ray_origin, ray_dir, max_t, &mut cb, [0.0; 3]);
