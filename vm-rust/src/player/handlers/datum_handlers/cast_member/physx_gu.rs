...
#[inline]
pub fn v_add(a: [f64; 3], b: [f64; 3]) -> [f64; 3] {
...
#[inline]
pub fn v_mul(a: [f64; 3], s: f64) -> [f64; 3] {
    [a[0] * s, a[1] * s, a[2] * s]
}
#[inline]
pub fn v_neg(a: [f64; 3]) -> [f64; 3] {
    [-a[0], -a[1], -a[2]]
}
#[inline]
...
#[inline]
pub fn v_len_sq(a: [f64; 3]) -> f64 {
    v_dot(a, a)
}
#[inline]
pub fn v_len(a: [f64; 3]) -> f64 {
    v_len_sq(a).sqrt()
}
#[inline]
...
pub fn contact_sphere_sphere(
    ...
    if distance_sq >= inflated * inflated {
        return None;
    }
...
pub fn contact_sphere_box(
    ...
    if d_rot[0] < -box_extents[0] {
        outside = true;
        d_rot[0] = -box_extents[0];
    } else if d_rot[0] > box_extents[0] {
        outside = true;
        d_rot[0] = box_extents[0];
    }
    if d_rot[1] < -box_extents[1] {
        outside = true;
        d_rot[1] = -box_extents[1];
    } else if d_rot[1] > box_extents[1] {
        outside = true;
        d_rot[1] = box_extents[1];
    }
    if d_rot[2] < -box_extents[2] {
        outside = true;
        d_rot[2] = -box_extents[2];
    } else if d_rot[2] > box_extents[2] {
        outside = true;
        d_rot[2] = box_extents[2];
    }
    ...
    if outside {
        ...
        let (loc_norm, mut separation) = if dist_to_surface[1] < dist_to_surface[0] {
            if dist_to_surface[1] < dist_to_surface[2] {
                (
                    [0.0, if d_rot[1] > 0.0 { 1.0 } else { -1.0 }, 0.0],
                    -dist_to_surface[1],
                )
            } else {
                (
                    [0.0, 0.0, if d_rot[2] > 0.0 { 1.0 } else { -1.0 }],
                    -dist_to_surface[2],
                )
            }
        } else if dist_to_surface[0] < dist_to_surface[2] {
            (
                [if d_rot[0] > 0.0 { 1.0 } else { -1.0 }, 0.0, 0.0],
                -dist_to_surface[0],
            )
        } else {
            (
...
fn generate_contacts_box_box(
    ...
    vtx[0].pos = p_minus_ex;
    vtx[2].pos = p_minus_ex;
    vtx[4].pos = p_minus_ex;
    vtx[6].pos = p_minus_ex;
    vtx[1].pos = p_plus_ex;
    vtx[3].pos = p_plus_ex;
    vtx[5].pos = p_plus_ex;
    vtx[7].pos = p_plus_ex;
    ...
    for face_idx in 0..6 {
        ...
        if !(q[0].penetrate && q[1].penetrate && q[2].penetrate && q[3].penetrate) {
            continue;
        }
        ...
        if (addflg & 1) == 0 {
            ...
            if x >= 0.0 {
                addflg |= 1;
                contact_buffer.add([x, -y1, -z1], contact_normal, -x);
            }
        }
        if (addflg & 2) == 0 {
            let x = is_in_yz(y1, -z1, &q);
...
pub fn contact_box_box(
    ...
    let axis00 = transform0.col0;
    let axis01 = transform0.col1;
    let axis02 = transform0.col2;
    let axis10 = transform1.col0;
    let axis11 = transform1.col1;
    let axis12 = transform1.col2;

    // Class I — face A.
    aaf_c[0][0] = v_dot(axis00, axis10);
    aaf_c[0][1] = v_dot(axis00, axis11);
    aaf_c[0][2] = v_dot(axis00, axis12);
    ...
    let mut d0 = extents0[0]
        + extents1[0] * aaf_abs_c[0][0]
        + extents1[1] * aaf_abs_c[0][1]
        + extents1[2] * aaf_abs_c[0][2];
    overlap[AXIS_A0 as usize] = d0 - d1[AXIS_A0 as usize].abs() + contact_distance;
    if overlap[AXIS_A0 as usize] < 0.0 {
        return 0;
    }

    aaf_c[1][0] = v_dot(axis01, axis10);
    aaf_c[1][1] = v_dot(axis01, axis11);
    aaf_c[1][2] = v_dot(axis01, axis12);
    ...
    d0 = extents0[1]
        + extents1[0] * aaf_abs_c[1][0]
        + extents1[1] * aaf_abs_c[1][1]
        + extents1[2] * aaf_abs_c[1][2];
    overlap[AXIS_A1 as usize] = d0 - d1[AXIS_A1 as usize].abs() + contact_distance;
    if overlap[AXIS_A1 as usize] < 0.0 {
        return 0;
    }

    aaf_c[2][0] = v_dot(axis02, axis10);
    aaf_c[2][1] = v_dot(axis02, axis11);
    aaf_c[2][2] = v_dot(axis02, axis12);
    ...
    d0 = extents0[2]
        + extents1[0] * aaf_abs_c[2][0]
        + extents1[1] * aaf_abs_c[2][1]
        + extents1[2] * aaf_abs_c[2][2];
    overlap[AXIS_A2 as usize] = d0 - d1[AXIS_A2 as usize].abs() + contact_distance;
    if overlap[AXIS_A2 as usize] < 0.0 {
        return 0;
    }
    ...
    d0 = extents1[0]
        + extents0[0] * aaf_abs_c[0][0]
        + extents0[1] * aaf_abs_c[1][0]
        + extents0[2] * aaf_abs_c[2][0];
    overlap[AXIS_B0 as usize] = d0 - d1[AXIS_B0 as usize].abs() + contact_distance;
    if overlap[AXIS_B0 as usize] < 0.0 {
        return 0;
    }

    d1[AXIS_B1 as usize] = v_dot(axis11, k_d);
    d0 = extents1[1]
        + extents0[0] * aaf_abs_c[0][1]
        + extents0[1] * aaf_abs_c[1][1]
        + extents0[2] * aaf_abs_c[2][1];
    overlap[AXIS_B1 as usize] = d0 - d1[AXIS_B1 as usize].abs() + contact_distance;
    if overlap[AXIS_B1 as usize] < 0.0 {
        return 0;
    }

    d1[AXIS_B2 as usize] = v_dot(axis12, k_d);
    d0 = extents1[2]
        + extents0[0] * aaf_abs_c[0][2]
        + extents0[1] * aaf_abs_c[1][2]
        + extents0[2] * aaf_abs_c[2][2];
    overlap[AXIS_B2 as usize] = d0 - d1[AXIS_B2 as usize].abs() + contact_distance;
    if overlap[AXIS_B2 as usize] < 0.0 {
        return 0;
    }
    ...
    if *pair_data == 0 {
        let tests = [
            (
                ...
                extents0[1] * aaf_abs_c[2][0]
                    + extents0[2] * aaf_abs_c[1][0]
                    + extents1[1] * aaf_abs_c[0][2]
                    + extents1[2] * aaf_abs_c[0][1],
            ),
            (
                af_ad[2] * aaf_c[1][1] - af_ad[1] * aaf_c[2][1],
                extents0[1] * aaf_abs_c[2][1]
                    + extents0[2] * aaf_abs_c[1][1]
                    + extents1[0] * aaf_abs_c[0][2]
                    + extents1[2] * aaf_abs_c[0][0],
            ),
            (
                af_ad[2] * aaf_c[1][2] - af_ad[1] * aaf_c[2][2],
                extents0[1] * aaf_abs_c[2][2]
                    + extents0[2] * aaf_abs_c[1][2]
                    + extents1[0] * aaf_abs_c[0][1]
                    + extents1[1] * aaf_abs_c[0][0],
            ),
            (
                af_ad[0] * aaf_c[2][0] - af_ad[2] * aaf_c[0][0],
                extents0[0] * aaf_abs_c[2][0]
                    + extents0[2] * aaf_abs_c[0][0]
                    + extents1[1] * aaf_abs_c[1][2]
                    + extents1[2] * aaf_abs_c[1][1],
            ),
            (
                af_ad[0] * aaf_c[2][1] - af_ad[2] * aaf_c[0][1],
                extents0[0] * aaf_abs_c[2][1]
                    + extents0[2] * aaf_abs_c[0][1]
                    + extents1[0] * aaf_abs_c[1][2]
                    + extents1[2] * aaf_abs_c[1][0],
            ),
            (
                af_ad[0] * aaf_c[2][2] - af_ad[2] * aaf_c[0][2],
                extents0[0] * aaf_abs_c[2][2]
                    + extents0[2] * aaf_abs_c[0][2]
                    + extents1[0] * aaf_abs_c[1][1]
                    + extents1[1] * aaf_abs_c[1][0],
            ),
            (
                af_ad[1] * aaf_c[0][0] - af_ad[0] * aaf_c[1][0],
                extents0[0] * aaf_abs_c[1][0]
                    + extents0[1] * aaf_abs_c[0][0]
                    + extents1[1] * aaf_abs_c[2][2]
                    + extents1[2] * aaf_abs_c[2][1],
            ),
            (
                af_ad[1] * aaf_c[0][1] - af_ad[0] * aaf_c[1][1],
                extents0[0] * aaf_abs_c[1][1]
                    + extents0[1] * aaf_abs_c[0][1]
                    + extents1[0] * aaf_abs_c[2][2]
                    + extents1[2] * aaf_abs_c[2][0],
            ),
            (
                af_ad[1] * aaf_c[0][2] - af_ad[0] * aaf_c[1][2],
                extents0[0] * aaf_abs_c[1][2]
                    + extents0[1] * aaf_abs_c[0][2]
                    + extents1[0] * aaf_abs_c[2][1]
                    + extents1[1] * aaf_abs_c[2][0],
            ),
        ];
        for (d, base) in tests.iter() {
            if d.abs() > contact_distance + base {
                return 0;
            }
        }
    }
    ...
    match min_index {
        x if x == AXIS_A0 => {
            if sign {
                ...
                trs.col0 = v_neg(axis00);
                trs.col1 = v_neg(axis01);
                trs.col2 = axis02;
                trs.p = v_add(transform0.p, v_mul(axis00, extents0[0]));
            }
            generate_contacts_box_box(
                buffer,
                ctc_nrm,
                extents0[1],
                extents0[2],
        ...
        x if x == AXIS_A1 => {
            ...
            if sign {
                ...
                trs.col0 = v_neg(axis01);
                trs.col1 = v_neg(axis02);
        ...
        x if x == AXIS_A2 => {
            ...
            if sign {
                ...
                trs.col0 = v_neg(axis02);
                trs.col1 = v_neg(axis00);
        ...
        x if x == AXIS_B0 => {
            if sign {
                ctc_nrm = axis10;
                trs.col0 = v_neg(axis10);
                trs.col1 = v_neg(axis11);
                trs.col2 = axis12;
        ...
        x if x == AXIS_B1 => {
            ...
            if sign {
                ctc_nrm = axis11;
                trs.col0 = v_neg(axis11);
                trs.col1 = v_neg(axis12);
        ...
        x if x == AXIS_B2 => {
            ...
            if sign {
                ctc_nrm = axis12;
                trs.col0 = v_neg(axis12);
                trs.col1 = v_neg(axis10);
...
pub fn distance_point_segment_sq_internal(
    p0: [f64; 3],
    dir: [f64; 3],
    point: [f64; 3],
) -> (f64, f64) {
...
pub fn distance_segment_segment_squared_centered(
    ...
    if f_det >= DSS_ZERO_TOLERANCE {
            ...
            if f_s0 <= f_ext_det0 {
                if f_s1 >= -f_ext_det1 {
                    if f_s1 <= f_ext_det1 {
                        ...
                        if f_tmp_s0 < -extent0 {
                            f_s0 = -extent0;
                            f_sqr_dist =
                                f_s0 * (f_s0 - 2.0 * f_tmp_s0) + f_s1 * (f_s1 + 2.0 * f_b1) + f_c;
                ...
                if f_s1 <= f_ext_det1 {
                    ...
                    if f_tmp_s1 < -extent1 {
                        f_s1 = -extent1;
                        f_sqr_dist =
                            f_s1 * (f_s1 - 2.0 * f_tmp_s1) + f_s0 * (f_s0 + 2.0 * f_b0) + f_c;
        ...
        if f_lambda < -f_e0p_e1 {
            f_lambda = -f_e0p_e1;
        } else if f_lambda > f_e0p_e1 {
            f_lambda = f_e0p_e1;
        }
...
pub fn distance_segment_segment_squared(
    ...
    if b1 {
        dir1 = v_mul(dir1, one_over_l1);
        length1 *= 0.5;
    }

    let (d2, p0, p1) =
        distance_segment_segment_squared_centered(center0, dir0, length0, center1, dir1, length1);
    let r0 = if b0 {
        (length0 + p0) * one_over_l0
    } else {
        0.0
    };
    let r1 = if b1 {
        (length1 + p1) * one_over_l1
    } else {
        0.0
    };
    (d2, r0, r1)
...
pub fn contact_sphere_capsule(
    ...
    Some(GuContact {
        point,
        normal,
        separation: sq_dist.sqrt() - radius_sum,
    })
}
...
pub fn contact_capsule_capsule(
    ...
    let (sq_dist, s, t) = distance_segment_segment_squared(
        seg_p0_a,
        v_sub(seg_p1_a, seg_p0_a),
        seg_p0_b,
        v_sub(seg_p1_b, seg_p0_b),
    );
    let radius_sum = radius0 + radius1;
    let inflated = radius_sum + contact_distance;
    let inflated_sq = inflated * inflated;
    if sq_dist >= inflated_sq {
        return 0;
    }
    ...
    if seg_len[0] != 0.0 {
        dir_norm[0] = v_mul(dir_norm[0], 1.0 / seg_len[0]);
    }
    if seg_len[1] != 0.0 {
        dir_norm[1] = v_mul(dir_norm[1], 1.0 / seg_len[1]);
    }
    ...
    if v_dot(dir_norm[0], dir_norm[1]).abs() > 0.9998 {
        ...
        if num_cons > 0 {
            return (buffer.count() - count_before) as i32;
        }
    }
