...
use super::physx_gu::{
    q_rotate, q_rotate_inv, v_add, v_cross, v_dot, v_len_sq, v_mul, v_neg, v_sub, GuContactBuffer,
};
...
pub fn polygonal_box(half_side: [f64; 3]) -> PolygonalData {
    ...
    polys[1].plane_n = [1.0, 0.0, 0.0];
    polys[1].plane_d = -half_side[0];
    polys[1].min_index = 0;
    polys[3].plane_n = [-1.0, 0.0, 0.0];
    polys[3].plane_d = -half_side[0];
    polys[3].min_index = 1;
    // Y axis (lines 471-474, 476-477).
    polys[4].plane_n = [0.0, 1.0, 0.0];
    polys[4].plane_d = -half_side[1];
    polys[4].min_index = 0;
    polys[5].plane_n = [0.0, -1.0, 0.0];
    polys[5].plane_d = -half_side[1];
    polys[5].min_index = 2;
    // Z axis (lines 485-488, 490-491).
    polys[2].plane_n = [0.0, 0.0, 1.0];
    polys[2].plane_d = -half_side[2];
    polys[2].min_index = 0;
    polys[0].plane_n = [0.0, 0.0, -1.0];
    polys[0].plane_d = -half_side[2];
    polys[0].min_index = 4;
...
pub fn polygonal_convex(verts: Vec<[f64; 3]>, faces: &[Vec<usize>]) -> Option<PolygonalData> {
    ...
    for f in faces.iter() {
        if f.len() < 3 || f.len() > 255 {
            return None;
        }
        let vref8 = refs.len() as u16;
        for &k in f {
            if k >= verts.len() || k > 255 {
                return None;
            }
            refs.push(k as u8);
        }
        ...
        if len <= 1e-10 {
            return None;
        } // degenerate face (3 collinear verts)
    ...
    for p in &polys {
        let dist = (p.plane_n[0] * centroid[0]
            + p.plane_n[1] * centroid[1]
            + p.plane_n[2] * centroid[2]
            + p.plane_d)
            .abs();
        if dist < radius {
            radius = dist;
...
fn test_separating_axis(
    ...
    if max0 + contact_distance < min1 || max1 + contact_distance < min0 {
        return None;
    }
    let d0 = max0 - min1;
    let d1 = max1 - min0;
...
fn clip_polygon_against_plane(
    ...
    let mut prev_dist =
        plane_n[0] * prev[0] + plane_n[1] * prev[1] + plane_n[2] * prev[2] + plane_d;
    for &cur in polygon {
...
fn generate_manifold(
    ...
    poly_a: &PolygonalData,
    poly_b: &PolygonalData,
    world_a: &GuBox,
    world_a_p: [f64; 3],
    world_b: &GuBox,
    world_b_p: [f64; 3],
    ...
    let ref_v0 = v_add(
        world_a
            .rotate(poly_a.verts[poly_a.polygon_vertex_refs[ref_polygon.vref8 as usize] as usize]),
        world_a_p,
    );
...
pub fn contact_hull_hull(
    ...
    for p in &poly0.polygons {
        ...
        let d = match test_separating_axis(
            poly0,
            poly1,
            &world0,
            pos0,
