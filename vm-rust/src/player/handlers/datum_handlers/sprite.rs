...
use crate::player::{
    ...
    owner_key_string, player_handle_scope_return, reserve_player_mut, reserve_player_ref,
...
mod tests {
    ...
    fn camera_foreign_view_name_returns_invalid_reference() {
        ...
        let result = session
            .with_player(1, |mut runtime| {
                ...
                scene
                    .nodes
                    .push(crate::director::chunks::w3d::types::W3dNode {
                        name: foreign_name,
                ...
                runtime.player.movie.score.channels[1].sprite.member =
                    Some(crate::player::cast_lib::CastMemberRef {
                        cast_lib: 1,
                        cast_member: 1,
                    });
                let receiver = runtime.player.alloc_datum(Datum::SpriteRef(1));
                SpriteDatumHandlers::call(&mut runtime, &receiver, "cameracount", &vec![])
            })
            .unwrap();
    ...
    fn flash_callback_fixture() -> (
        ...
        assert!(session
            .borrow_mut()
            .add_player(1, async_std::channel::unbounded().0));
        let (owner, receiver, handler) = session
            ...
            .with_player(1, |context| {
                ...
                context.player.movie.score.sprite_spans =
                    vec![crate::player::score::ScoreSpriteSpan {
                        channel_number: 1,
                        start_frame: 1,
                        end_frame: 1,
                        scripts: vec![],
        ...
        session
            ...
            .with_player(1, |context| {
                ...
                vec![
                    ...
                    context
                        .player
                        .alloc_datum(Datum::String("callback".to_owned())),
    ...
    fn set_callback_short_arity_returns_void_without_generation_or_host_work() {
        ...
        let (generation, actions) = session
            ...
            .with_player(1, |context| {
                (
                    context.player.flash_instance_generation(1),
        ...
        assert!(
            actions.is_empty(),
            "short setCallback must not prepare host work"
        );
    }
        ...
        session
            ...
            .with_player(1, |mut runtime| {
                ...
                let args = vec![
                    runtime
                        .player
                        .alloc_datum(Datum::String("_root".to_owned())),
                    runtime
                        .player
                        .alloc_datum(Datum::String("callback".to_owned())),
                ...
                let error =
                    SpriteDatumHandlers::call(&mut runtime, &receiver, "setCallback", &args)
                        .expect_err(
                            "synchronous setCallback must require the owner-bound async path",
                        );
                assert_eq!(
                    error.message,
                    "setCallback requires the owner-bound async path"
                );
    ...
    fn owned_sprite_call_preparation_preserves_path_and_json_string_only() {
        ...
        match request.operation {
            crate::player::handlers::datum_handlers::flash_object::FlashOperation::Call {
                args_xml,
            } => {
    ...
    fn set_callback_rejects_foreign_stale_and_different_sprite_objects_before_host_work() {
        ...
        for (label, object_owner, generation, sprite_num) in cases {
            ...
            assert!(
                actions.is_empty(),
                "{label} callback rejection emitted host work"
            );
        }
    }
    ...
    fn sprite_set_variable_checks_consumed_arguments_before_host_work() {
        ...
        let path_only = session
            ...
            .with_player(1, |context| {
                vec![context
                    .player
                    .alloc_datum(Datum::String("value".to_owned()))]
            })
            .expect("callback fixture player exists");
        let error = async_std::task::block_on(SpriteDatumHandlers::execute_async_request(
            session.clone(),
        ...
        assert!(
            actions.is_empty(),
            "argument rejection must precede host work"
        );
    }
        ...
        let args = session
            ...
            .with_player(1, |context| {
                vec![
                    ...
                    context
                        .player
                        .alloc_datum(Datum::String("ignored".to_owned())),
                ]
            })
            .expect("callback fixture player exists");
        let error = async_std::task::block_on(SpriteDatumHandlers::execute_async_request(
                ...
                .with_player(1, |context| {
                    if object_mode {
                        vec![
                            context
                                .player
                                .alloc_datum(Datum::String("_root.callbackProbeString".to_owned())),
    ...
    fn sprite_get_variable_absent_object_binds_and_reuses_pending_generation() {
            ...
            session
                ...
                .with_player(1, |context| {
                    vec![
                        context
                            .player
                            .alloc_datum(Datum::String("early".to_owned())),
                        context.player.alloc_datum(Datum::Int(0)),
                    ]
                })
                .expect("callback fixture player exists")
        ...
        assert_eq!(
            first_object.owner_key,
            Some(crate::player::owner_key_string(&owner))
        );
    ...
    fn sprite_get_variable_unresolved_object_binds_exact_pair_without_host_work() {
        ...
        let object = session
            .borrow_mut()
            .with_player(1, |context| {
                context.player.get_datum(&result).as_flash_object().cloned()
            })
            .expect("callback fixture player exists")
...
fn set_sprite_camera_list(
    ...
    sprite.w3d_camera = if cams.is_empty() {
        None
    } else {
        Some(cams.remove(0))
    };
...
fn prepare_sprite_call_inputs(
    ...
    let args_xml = args
        .get(1)
        .map(|arg| {
            checked_player_datum(player, symbols, arg).and_then(|datum| datum.string_value(symbols))
        })
        .transpose()?
        .unwrap_or_default();
    Ok((path, args_xml))
...
fn checked_sprite_num(
    runtime: &mut ExecutionContext<'_>,
    datum: &DatumRef,
) -> Result<i16, ScriptError> {
...
impl SpriteDatumHandlers {
    fn prepare_lingo_callback(
        ...
        if args.len() < 3 {
            return Err(ScriptError::new(
                "setCallback requires a Flash object, method, and handler".to_owned(),
            ));
        }
    ...
    async fn execute_flash_variable_request(
        ...
        let actions =
            crate::player::bind_flash_host_actions(actions, session.clone(), request.player_id);
    ...
    pub(crate) async fn execute_async_request(
        ...
        let (handler_name, flash_sprite) = session
            ...
            .with_player(
                request.player_id,
                |context| -> Result<(String, Option<i32>), ScriptError> {
                    if !request.owner.same_identity(&context.player.owner)
                        || !request.owner.is_arena_live()
                    {
                        return Err(crate::player::cancelled_scope_error());
                    ...
                    .and_then(|(sprite_num, cast_lib, cast_member)| {
                        context
                            ...
                            .filter(|member| matches!(member.member_type, CastMemberType::Flash(_)))
                            .map(|_| sprite_num)
                    });
        ...
        if handler_lower == "setcallback" {
            ...
            let Some(sprite_num) = flash_sprite.map(|number| number as i16) else {
                return Err(ScriptError::new(
                    "setCallback requires a Flash sprite".to_owned(),
                ));
            };
            ...
            let (registration, generation, actions) = session
                ...
                .with_player(request.player_id, |context| -> Result<_, ScriptError> {
                    ...
                    let generation = generation.ok_or_else(|| {
                        ScriptError::new(
                            "Flash callback binding has no current generation".to_owned(),
                        )
                    })?;
                    if let Some(target_ref) = registration.lc_target.clone() {
                        context.player.flash_lc_callbacks.insert(
                            (
                                registration.translated_path.clone(),
                                registration.flash_method.clone(),
                            ),
        ...
        if handler_lower == "getvariable" || handler_lower == "setvariable" {
            return Self::execute_flash_variable_request(session, &request, &handler_name).await;
        }
        if let Some(sprite_num) = flash_sprite {
            let json_args = session
                ...
                .with_player(
                    request.player_id,
                    |context| -> Result<String, ScriptError> {
                ...
                .ok_or_else(crate::player::cancelled_scope_error)??;
            match ruffle_call_function(sprite_num, &root_flash_path(&handler_name), &json_args) {
                Ok(value) => {
                ...
                Err(error) => {
                    warn!(
                        "sprite direct Flash method '{}' error: {:?}",
                        handler_name, error
                    );
                }
            }
    ...
    fn resolve_sprite_flash_member(
        datum: &DatumRef,
    ...
    fn resolve_sprite_flash_member_explicit(
        ...
        Ok(sprite
            .member
            .as_ref()
            .map(|member| (sprite_num as i32, member.cast_lib, member.cast_member)))
    }
    ...
    pub fn has_async_handler(datum: &DatumRef, handler_name: &str) -> Result<bool, ScriptError> {
        ...
        if matches!(
            ...
            let ready =
                reserve_player_ref(|player| match player.get_datum(datum).to_sprite_ref() {
    ...
    pub fn call(
        ...
        match name_lower.as_str() {
            ...
            "cameracount" => {
                ...
                let player = &mut *runtime.player;
                {
                    let count = sprite_camera_list(player, &*runtime.symbols, sprite_num as i16)?
                        .len()
                        .max(1) as i32;
            ...
            "addcamera" => {
                ...
                let player = &mut *runtime.player;
                {
                    ...
                    let cam_entry = if !args.is_empty() {
                        match player.get_datum(&args[0]) {
                            Datum::Shockwave3dObjectRef(r) => {
                                Some(crate::player::sprite::SpriteCamera {
                    ...
                    if let Some(cam_entry) = cam_entry {
                        ...
                        let mut cams =
                            sprite_camera_list(player, &*runtime.symbols, sprite_num as i16)?;
                        let at = match index {
                ...
                let player = &mut *runtime.player;
                {
                    ...
                    let at = match &target {
                        Some(Datum::Shockwave3dObjectRef(r)) => {
                            let target_name = runtime.symbols.lower(&r.name).map_err(|_| {
                                crate::player::symbols::symbol::SymbolError::Foreign
                            })?;
                            cams.iter()
                                .position(|c| target_name.eq_ignore_ascii_case(&c.name))
                        }
                        Some(Datum::String(name)) => {
                            cams.iter().position(|c| c.name.eq_ignore_ascii_case(name))
                        }
                        Some(other) => other.int_value().ok().map(|i| (i.max(1) as usize) - 1),
                        None => None,
                    };
                    if let Some(at) = at {
                        if at < cams.len() {
            ...
            "getat" | "getaprop" => {
                ...
                let symbols = &mut *runtime.symbols;
                {
                    ...
                    let instance_refs =
                        crate::player::score::get_sprite_script_instance_ids_checked(
                            player,
                            &*symbols,
                            sprite_num,
                            fallback.as_slice(),
            ...
            "gotoframe" => {
                if let Some((sn, _cl, _cm)) = Self::resolve_sprite_flash_member_explicit(
                    runtime.player,
                    runtime.symbols,
                    datum,
                )? {
            ...
            "hittest" => {
                if let Some((sn, _cl, _cm)) = Self::resolve_sprite_flash_member_explicit(
                    ...
                    let (sx, sy) = runtime.with_player_and_symbols(
                        |player, symbols| -> Result<(i32, i32), ScriptError> {
                            match player.get_datum(&args[0]) {
            ...
            "getflashproperty" => {
                if let Some((sn, _cl, _cm)) = Self::resolve_sprite_flash_member_explicit(
                    ...
                    let target = runtime.with_player_and_symbols(|player, symbols| {
                        player.get_datum(&args[0]).string_value(symbols)
                    })?;
                    let prop_num = runtime.with_player_and_symbols(|player, symbols| {
            ...
            "setflashproperty" => {
                if let Some((sn, _cl, _cm)) = Self::resolve_sprite_flash_member_explicit(
                    ...
                    let value = runtime.with_player_and_symbols(|player, symbols| {
                        player.get_datum(&args[2]).string_value(symbols)
                    })?;
