...
fn add_2_3_returns_5() {
    ...
    let result = rt
        .invoke(
            &add,
            vec![JsValue::Int(2), JsValue::Int(3)],
            JsValue::Undefined,
        )
        .expect("invoke add");
    match result {
...
fn add_with_strings_concatenates() {
    ...
    let result = rt
        .invoke(
            &add,
            vec![
                JsValue::String(Rc::new("hello ".into())),
                JsValue::String(Rc::new("world".into())),
            ],
            JsValue::Undefined,
        )
        .expect("invoke");
...
fn program_hoists_globals_and_inits_them() {
    ...
    assert!(
        matches!(counter, Some(JsValue::Int(0))),
        "pCounter = 0, got {:?}",
        counter
    );
    // var pName = "alice"
...
fn on_preparemovie_invokes_trace_with_correct_string() {
    ...
    let on_prepare = rt
        ...
        .cloned()
        .expect("hoisted");
    rt.invoke(&on_prepare, vec![], JsValue::Undefined)
        .expect("invoke");
...
fn array_literal_and_length_property() {
    ...
    let on_mouseup = rt
        ...
        .cloned()
        .expect("hoisted");
    rt.invoke(&on_mouseup, vec![], JsValue::Undefined)
        .expect("invoke");
...
fn nested_interpreted_call_shares_budget_and_restores_depth_after_throw() {
    ...
    rt.global.borrow_mut().set_own(
        "inner",
        interpreted_inner_function(vec![JsOp::One as u8, JsOp::Return as u8], "inner"),
    );
    ...
    assert_eq!(
        remaining,
        max - 10,
        "nested invoke must share the outer budget"
    );

    rt.global.borrow_mut().set_own(
        "inner",
        interpreted_inner_function(vec![JsOp::Push as u8, JsOp::Throw as u8], "throwing_inner"),
    );
    ...
    assert_eq!(
        remaining,
        max - 10,
        "a fresh outer call must reset the budget"
    );
}
...
fn synth_arithmetic_chain() {
    ...
    let mut bc = Vec::new();
    bc.push(JsOp::Number as u8);
    bc.extend(u16_be(0)); // push 2
    bc.push(JsOp::Number as u8);
    bc.extend(u16_be(1)); // push 3
    bc.push(JsOp::Number as u8);
    bc.extend(u16_be(2)); // push 4
    bc.push(JsOp::Mul as u8); // 3*4 = 12
    bc.push(JsOp::Add as u8); // 2+12 = 14
    bc.push(JsOp::Number as u8);
    bc.extend(u16_be(3)); // push 1
...
fn synth_jump_and_compare() {
    ...
    bc.push(JsOp::Number as u8);
    bc.extend(u16_be(0)); // push 5  @ pc=0
    bc.push(JsOp::Number as u8);
    bc.extend(u16_be(1)); // push 10 @ pc=3
    bc.push(JsOp::Gt as u8); // 5 > 10 = false  @ pc=6
    let ifeq_pc = bc.len(); // @ pc=7
    bc.push(JsOp::Ifeq as u8);
    bc.extend(i16_be(0)); // placeholder offset
...
fn synth_string_concat_and_compare() {
    ...
    let mut bc = Vec::new();
    bc.push(JsOp::String as u8);
    bc.extend(u16_be(0)); // push "foo"
    bc.push(JsOp::String as u8);
    bc.extend(u16_be(1)); // push "bar"
    bc.push(JsOp::Add as u8); // "foobar"
    bc.push(JsOp::String as u8);
    bc.extend(u16_be(2)); // push "foobar"
...
fn synth_tableswitch_routes_to_correct_case() {
    ...
    let mut bc = Vec::new();
    bc.push(JsOp::Number as u8);
    bc.extend(u16_be(0)); // push 1 @ pc=0..2
    ...
    let case0_at = bc.len();
    bc.push(JsOp::Number as u8);
    bc.extend(u16_be(1)); // push 100
    bc.push(JsOp::Return as u8);
    let case1_at = bc.len();
    bc.push(JsOp::Number as u8);
    bc.extend(u16_be(2)); // push 200
    bc.push(JsOp::Return as u8);
    let default_at = bc.len();
    bc.push(JsOp::Number as u8);
    bc.extend(u16_be(3)); // push 999
    bc.push(JsOp::Return as u8);
    ...
    assert!(
        matches!(result, JsValue::Int(200)),
        "expected 200, got {:?}",
        result
    );
}
...
fn synth_forin_counts_keys() {
    ...
    bc.push(JsOp::Setvar as u8);
    bc.extend(u16_be(1)); // pc=1: count = 0 (slot 1)
    ...
    bc.push(JsOp::Getarg as u8);
    bc.extend(u16_be(0)); // pc=6: push obj

    // Loop top: FORVAR k
    let loop_top = bc.len(); // pc=9
    bc.push(JsOp::Forvar as u8);
    bc.extend(u16_be(0)); // pc=9: FORVAR slot=0 (k)

    let ifeq_pc = bc.len(); // pc=12
    bc.push(JsOp::Ifeq as u8);
    bc.extend(i16_be(0)); // pc=12: placeholder, patched below

    // Body: count++
    bc.push(JsOp::Incvar as u8);
    bc.extend(u16_be(1)); // pc=15
    bc.push(JsOp::Pop as u8); // pc=18: discard the incremented value

    // Backward goto to loop_top
    let goto_pc = bc.len(); // pc=19
    bc.push(JsOp::Goto as u8);
    bc.extend(i16_be(0)); // pc=19: placeholder
    ...
    bc.push(JsOp::Getvar as u8);
    bc.extend(u16_be(1)); // push count
    bc.push(JsOp::Return as u8);
    ...
    let mut rt = JsRuntime::new();
    let result = rt
        .call_function(&Rc::new(f), vec![obj_val], JsValue::Undefined)
        .unwrap();
    assert!(
        matches!(result, JsValue::Int(3)),
        "expected 3 keys, got {:?}",
        result
    );
}
...
fn synth_forin_array_walks_indices() {
    ...
    bc.push(JsOp::Setvar as u8);
    bc.extend(u16_be(1)); // pc=1: n = undefined
    bc.push(JsOp::Pop as u8); // pc=4

    bc.push(JsOp::Push as u8); // pc=5: iter placeholder
    bc.push(JsOp::Getarg as u8);
    bc.extend(u16_be(0)); // pc=6

    let loop_top = bc.len();
    bc.push(JsOp::Forvar as u8);
    bc.extend(u16_be(0)); // pc=9: writes k
    let ifeq_pc = bc.len();
    bc.push(JsOp::Ifeq as u8);
    bc.extend(i16_be(0));
    ...
    // n = k
    bc.push(JsOp::Getvar as u8);
    bc.extend(u16_be(0)); // push k
    bc.push(JsOp::Setvar as u8);
    bc.extend(u16_be(1)); // n = k (leaves k on stack)
    ...
    let arr = JsArray {
        items: vec![JsValue::Int(10), JsValue::Int(20), JsValue::Int(30)],
    };
    let arr_val = JsValue::Array(Rc::new(RefCell::new(arr)));

    let mut rt = JsRuntime::new();
    let result = rt
        .call_function(&Rc::new(f), vec![arr_val], JsValue::Undefined)
        .unwrap();
    match result {
...
fn synth_closure_captures_outer_arg() {
    ...
    inner_bc.push(JsOp::Name as u8);
    inner_bc.extend(u16_be(0)); // push x (via NAME)
    ...
    outer_bc.push(JsOp::Anonfunobj as u8);
    outer_bc.extend(u16_be(0)); // push closure-over-current-scope
    ...
    let result = rt
        .invoke(&closure, Vec::new(), JsValue::Undefined)
        .expect("inner call");
    assert!(
        matches!(result, JsValue::Int(42)),
        "expected 42 from captured x, got {:?}",
        result
    );
}
...
fn synth_try_catch_recovers_thrown_value() {
    ...
    let mut rt = JsRuntime::new();
    let result = rt
        .call_function(&Rc::new(f), Vec::new(), JsValue::Undefined)
        .unwrap();
    assert!(
        matches!(result, JsValue::Int(42)),
        "catch should recover the thrown value, got {:?}",
        result
    );
}
...
fn synth_try_catch_no_throw_returns_value() {
    ...
    bc.push(JsOp::Number as u8);
    bc.extend(u16_be(0)); // push 7
    bc.push(JsOp::Return as u8); // return 7
    let try_end = bc.len();

    let catch_start = bc.len();
    bc.push(JsOp::Number as u8);
    bc.extend(u16_be(1)); // push 9
    bc.push(JsOp::Return as u8);
    ...
    let result = rt
        ...
        .unwrap();
    assert!(
        matches!(result, JsValue::Int(7)),
        "no-throw path returns from try body, got {:?}",
        result
    );
}
...
fn synth_pre_increment_var() {
    ...
    let mut bc = Vec::new();
    bc.push(JsOp::Number as u8);
    bc.extend(u16_be(0)); // push 5
    bc.push(JsOp::Setvar as u8);
    bc.extend(u16_be(0)); // x = 5 (leaves 5 on stack)
    bc.push(JsOp::Pop as u8);
    bc.push(JsOp::Incvar as u8);
    bc.extend(u16_be(0)); // ++x → pushes new value
    bc.push(JsOp::Return as u8);
    let ir = super::xdr::JsScriptIR {
        magic: 0xdead_0003,
        bytecode: bc,
    ...
    let result = rt
        .call_function(&Rc::new(f), vec![], JsValue::Undefined)
        .unwrap();
    assert!(
        matches!(result, JsValue::Int(6)),
        "expected 6, got {:?}",
        result
    );
}
...
fn bridge_trace_alone_for_on_preparemovie() {
    ...
    let on_prep = rt
        ...
        .cloned()
        .expect("hoisted");
    rt.invoke(&on_prep, vec![], JsValue::Undefined)
        .expect("invoke");
...
fn run_with_stdlib(
    ...
    atoms: Vec<JsAtom>,
    nvars: u16,
) -> Result<JsValue, super::value::JsError> {
    let ir = super::xdr::JsScriptIR {
        magic: 0xdead_0003,
        bytecode: bc,
        prolog_length: 0,
...
fn stdlib_math_floor_ceil_round() {
    ...
    let mut bc = Vec::new();
    bc.push(JsOp::Name as u8);
    bc.extend(u16_be(0)); // push Math
    bc.push(JsOp::Getprop as u8);
    bc.extend(u16_be(1)); // .floor
    bc.push(JsOp::Pushobj as u8); // this (we'll allow)
    bc.push(JsOp::Number as u8);
    bc.extend(u16_be(2)); // push 3.7
    bc.push(JsOp::Call as u8);
    bc.extend(u16_be(1));
    bc.push(JsOp::Return as u8);
...
fn stdlib_math_pi_and_sqrt() {
    let atoms = vec![JsAtom::String("Math".into()), JsAtom::String("PI".into())];
    let mut bc = Vec::new();
    bc.push(JsOp::Name as u8);
    bc.extend(u16_be(0));
    bc.push(JsOp::Getprop as u8);
...
fn stdlib_parseint_radix_and_default() {
    ...
    let r1 = rt
        .invoke(
            &pi,
            vec![JsValue::String(Rc::new("42".into()))],
            JsValue::Undefined,
        )
        .unwrap();
    ...
    let r2 = rt
        .invoke(
            &pi,
            vec![JsValue::String(Rc::new("0xff".into()))],
            JsValue::Undefined,
        )
        .unwrap();
    ...
    let r3 = rt
        .invoke(
            &pi,
            vec![JsValue::String(Rc::new("101".into())), JsValue::Int(2)],
            JsValue::Undefined,
        )
        .unwrap();
...
fn stdlib_isnan_recognises_nan() {
    ...
    assert!(matches!(
        rt.invoke(
            &f,
            vec![JsValue::String(Rc::new("not a number".into()))],
            JsValue::Undefined
        )
        .unwrap(),
        JsValue::Bool(true)
    ...
    assert!(matches!(
        rt.invoke(&f, vec![JsValue::Int(7)], JsValue::Undefined)
            .unwrap(),
        JsValue::Bool(false)
    ));
}
...
fn stdlib_array_push_pop_join() {
    let mut rt = JsRuntime::with_stdlib();
    let arr = JsValue::Array(Rc::new(std::cell::RefCell::new(super::value::JsArray {
        items: vec![JsValue::Int(1), JsValue::Int(2)],
    })));
    // push(3, 4)
    ...
    bc.push(JsOp::Setvar as u8);
    bc.extend(u16_be(0)); // a = [1]
    bc.push(JsOp::Pop as u8);
    bc.push(JsOp::Getvar as u8);
    bc.extend(u16_be(0)); // load a
    bc.push(JsOp::Getprop as u8);
    bc.extend(u16_be(1)); // .push
    bc.push(JsOp::Getvar as u8);
    bc.extend(u16_be(0)); // this = a
    bc.push(JsOp::Number as u8);
    bc.extend(u16_be(0)); // arg = ... actually push the int 99
    ...
    bc.pop();
    bc.pop();
    bc.pop(); // pop the NUMBER op + 2 operand bytes
    bc.push(JsOp::Uint16 as u8);
    bc.extend(u16_be(99)); // push 99
    bc.push(JsOp::Call as u8);
    bc.extend(u16_be(1)); // a.push(99) -> 2
    bc.push(JsOp::Pop as u8);
    bc.push(JsOp::Getvar as u8);
    bc.extend(u16_be(0)); // return a
    bc.push(JsOp::Return as u8);
...
fn stdlib_string_to_upper_and_length() {
    ...
    match r {
        JsValue::String(ss) => assert_eq!(&*ss, "HELLO"),
        other => panic!("{:?}", other),
    };
...
fn synth_extended_jump_offsets() {
    ...
    bc.push(JsOp::Number as u8);
    bc.extend(u16_be(0)); // 5..7 — never executes (offset>=7 jumps past)
    bc.push(JsOp::Number as u8);
    bc.extend(u16_be(0)); // 7..9 push 42
...
fn synth_array_literal_via_initelem() {
    ...
    let atoms = vec![JsAtom::Int(10), JsAtom::Int(20), JsAtom::Int(30)];
    ...
    bc.push(JsOp::Name as u8);
    bc.extend(u16_be(3)); // NAME atom_idx 3 = "Array"
    bc.push(JsOp::Pushobj as u8);
...
fn decompile_nested_short_circuit_expressions() {
    ...
    for op in [JsOp::And, JsOp::Or] {
        ...
        bc.push(JsOp::Name as u8);
        bc.extend(u16_be(0)); // push a
        let outer_pc = bc.len() as i32;
        bc.push(op as u8);
        bc.extend(i16_be((join - outer_pc) as i16));
        bc.push(JsOp::Name as u8);
        bc.extend(u16_be(1)); // push b
        let inner_pc = bc.len() as i32;
        bc.push(op as u8);
        bc.extend(i16_be((join - inner_pc) as i16));
        bc.push(JsOp::Name as u8);
        bc.extend(u16_be(2)); // push c
